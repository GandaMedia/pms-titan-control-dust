<template>
  ...
  <ul role="list" class="grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 sm:gap-x-6 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 xl:gap-x-8">
    <li v-for="button in effects" :key="button.source" class="relative">
      <div
          class="group block w-full aspect-w-10 aspect-h-7 rounded-lg bg-gray-100 overflow-hidden">
        <img :src="button.source" alt="" class="object-cover pointer-events-none "/>
        <button type="button" class="absolute inset-0 focus:outline-none"
                :class="[button.active ? 'bg-green-500 opacity-50' : '', button.down && button.type !== 'toggle' ? 'bg-black opacity-50' : '']"
                @mousedown="pushEffect(button)" @mouseup="releaseEffect(button)">
          <span class="sr-only">View details for {{ button.title }}</span>
        </button>
      </div>
      <p class="mt-2 block text-sm font-medium text-gray-900 truncate pointer-events-none">{{ button.title }}</p>
    </li>
  </ul>
